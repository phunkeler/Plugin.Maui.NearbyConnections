...
/// warns breaks on a future OS. Stopping them keeps <see cref="INearbySession.IsAdvertising"/> and
/// <see cref="INearbySession.IsDiscovering"/> honest: while suspended nothing is scanning, so
...
/// connections raise <see cref="INearbySession.ConnectionDropped"/> and the flags go
...
{
    readonly NearbySession _session;
    readonly ILogger _logger;

    NSObject? _backgroundRegistration;
    int _disposeGuard;

    internal AppLifecycleObserver(NearbySession session, ILogger logger)
    {
