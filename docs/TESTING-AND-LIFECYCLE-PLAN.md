...
Unit tests substitute `INearbyConnections` with `FakeNearbyConnections`, so **everything below the
...
**Goal:** exercise the real `NearbySession` against a scriptable in-memory transport, so the
...
> `UIApplication.DidEnterBackgroundNotification` and calls `NearbySession.StopAsync`. See §3.7 for
...
`NearbySession` and disposed before `StopAsync` during session disposal, so a notification arriving
