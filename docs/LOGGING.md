...
| `1015` | `Warning` | An automatically accepted request never completed its handshake and was abandoned at its bound — `min(the offer's remaining window, 30s)`. No application code initiated the accept, so this log is the only record. |
...
IDs you will see routinely rather than alert on: `1012` (`Debug`) records an inbound request
lapsing because the application did not answer it before the offer's deadline — the window the
initiating device declared via its own `ConnectTimeout`, clamped by the library. That is an
ordinary outcome, not a fault. `2095` (`Debug`) records a connection request that carried no valid
connect-request frame (a peer on an older plugin version), so the default 30s window was assumed;
`2096` (`Debug`) records a declared offer window being clamped to the internal five-minute trust
bound.
...
**Device display names never appear in log messages or exception messages** — local or remote.
They are user-chosen and often personal ("Sam's iPhone"), so they are identity data; every message
carries the opaque `DeviceId` instead. File paths still appear, at `Error`/`Debug`.
...
name itself still reaches the sink as the remote peer wrote it. Treat it as untrusted identity
data.
