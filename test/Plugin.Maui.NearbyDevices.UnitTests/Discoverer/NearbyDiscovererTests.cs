...
{
    // ---------------------------------------------------------------------------
    // FakeNearbyDevices — discover channel + ConnectAsync TCS backed by tests.
    // ---------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------------
    sealed class MultiConnectFake(params NearbyConnection[] connections) : INearbyDevices
    {
        int _index;
    ...
    // ---------------------------------------------------------------------------
    static INearbyDevices CreateSubstitute(Channel<NearbyDeviceEvent>? discoverChannel = null)
    {
    ...
    // ---------------------------------------------------------------------------
    // ObservableDiscoverFake — an INearbyDevices whose DiscoverAsync enumerator
    ...
    // ---------------------------------------------------------------------------
    sealed class ObservableDiscoverFake : INearbyDevices
    {
        readonly Channel<NearbyDeviceEvent> _discoverChannel =
            Channel.CreateUnbounded<NearbyDeviceEvent>();
    ...
    static async Task<(NearbyDiscoverer Discoverer, NearbyConnection Connection, Channel<NearbyPayload> ReceiveChannel)>
        SetupConnectedDiscoverer(FakeNearbyDevices fake, NearbyDevice? device = null)
    {
    ...
    public sealed class EventStreamBehavior
    {
        ...
        // of the service. Instead, StopAsync emits DeviceLost for all visible devices so
        // subscribers can clear their UI without restarting the event loop.
        [TestMethod]
        public async Task StopAsync_EmitsDeviceLost_ForVisibleDevices()
        {
            // Arrange
            var fake = new FakeNearbyDevices();
            var discoverer = new NearbyDiscoverer(fake);
            ...
            // Act
            await discoverer.StopAsync();
            await WaitForAsync(() => received.Any(e => e is DiscovererEvent.DeviceLost));

            await cts.CancelAsync();
            await Task.WhenAny(streamTask, Task.Delay(TimeSpan.FromSeconds(1)));

            // Assert — DeviceLost emitted; stream stays open (not completed by StopAsync)
            Assert.IsTrue(received.Any(e => e is DiscovererEvent.DeviceLost));
        ...
        public async Task StartAsync_AfterStop_ProducesFreshStream()
        {
            ...
            {
                await foreach (var ev in discoverer.EventsAsync(cts1.Token))
                {
            ...
            {
                await foreach (var ev in discoverer.EventsAsync(cts2.Token))
                {
        ...
        // A subscriber that starts after a device was found receives it exactly once via
        ...
        {
            // Arrange — device found while no consumer is active
            var fake = new FakeNearbyDevices();
            var discoverer = new NearbyDiscoverer(fake);
            await discoverer.StartAsync();
            ...
            // Act — subscribe after the device is already in the snapshot
            using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(5));
            var received = new List<DiscovererEvent>();
            ...
            // Assert — exactly one DeviceFound from snapshot replay, not duplicated
            Assert.AreEqual(1, received.OfType<DiscovererEvent.DeviceFound>().Count(),
                "Late subscriber must receive the snapshot event exactly once.");

            await discoverer.StopAsync();
        }

        // When the platform completes INearbyDevices.DiscoverAsync with an error,
        ...
        public async Task EventsAsync_TwoConcurrentConsumers_BothReceiveAllEvents()
        {
            ...
            Assert.AreEqual(EventCount, consumer1Events.Count, "Consumer 1 should receive all events.");
            Assert.AreEqual(EventCount, consumer2Events.Count, "Consumer 2 should receive all events.");
        }
    }
}
